...
var AXTree = Class.create(AXJ, {
	...
	initialize: function (AXJ_super) {
		...
		this.config.multiSelect = false; 	//multiSelect(ctrl) 옵션 추가 2014.06.11
		this.config.maxWidth = 0; 					// 조직도의 폭
		this.config.textLengthConstant = 9.5; 		//문자 수에 따라 폭이 조절되는 정도
	},
	/* 공통 영역 */
	defineConfig: function (rewrite) {
		...
		if (cfg.fitToWidth) { //너비 자동 맞춤버전의 경우
			var bodyWidth = this.body.width() - 0;
		...
		// col너비 합계 구하기 ~~~~~~~~~~~~~~ 구해진 너비합은 트리 head, body 의 너비로 지정됨.
	...
	init: function () {
		...
		if (cfg.width) treeCss.push("width:100%;");
		if (cfg.height) treeCss.push("height:100%;");
	...
	resetHeight: function () {
		...
		var colHeadHeight = 0;
		try {
			colHeadHeight = ((cfg.colHead || {}).display) ? this.colHead.outerHeight() : 0;
		} catch (e) {
		}
		...
		try {
			...
			var scrollTrackYHeight = (cfg.xscroll) ? bodyHeight - axdom("#" + cfg.targetID + "_AX_scrollTrackXY").outerHeight() : bodyHeight;
			axdom("#" + cfg.targetID + "_AX_scrollTrackY").css({ height: scrollTrackYHeight });

			var scrollYHandleHeight = ((bodyHeight - axdom("#" + cfg.targetID + "_AX_scrollTrackXY").outerHeight()) * scrollTrackYHeight) / scrollHeight;
	...
	setBody: function (list) {
		...
		po.push("<div class=\"tdRelBlock\">");
		po.push("<div class=\"bodyNode bodyTdText\" id=\"statusText\" align=\"center\">");
		//po.push("Loading..");
		po.push("</div>");
		po.push("</div>");
		po.push("</td>");
	...
	getItem: function (itemIndex, item, isfix, hasTr) {
		...
		for (var r = 0; r < cfg.body.rows.length; r++) {
			...
			if (hasTrValue) tpo.push("<tr class=\"gridBodyTr gridBodyTr_" + itemIndex + " " + evenClassName + " " + hashClassName + " " + trAddClass + "\" id=\"" + cfg.targetID + "_AX_tr_" + r + "_AX_" + (isfix || "n") + "_AX_" + itemIndex + "\" style=\"" +"display: block;" + trStyles.join('') + "\">"); //좀전 수정
			var colCount = 0;
			axf.each(cfg.body.rows[r], function (CHidx, CH) {
				if (CH.display && CH.colspan > 0) {
					...
					if (isfix == "n" || (isfix != undefined && colCount < (cfg.fixedColSeq + 1))) {
						...
						if (CH.indent) {
							...
							if (CH.align == "left") {
								bodyNodeStyles.push("padding-left:" + (indentWidth + iconWidth - 21) + "px;");  // -21 추가
							} else {
						...
						if(item.type == 'root'){
							tpo.push("<td" + valign + rowspan + colspan + " id=\"" + cfg.targetID + "_AX_" + (isfix || "n") + "body_AX_" + r + "_AX_" + CHidx + "_AX_" + itemIndex + "\" class=\"rootTd" + bottomClass + fixedClass + "\">");	
						}else{
						...
						if (CH.indent) {
							...
							if(cfg.showConnectionLine){
								...
								//-20에서 -40으로 변경, iconClass 추가
								iconClass = getIconClassValue(CH.getIconClass, item, itemIndex, item[CH.key], CH.key, CH);
								tpo.push("<a class=\"bodyNodeIndent " + iconClass + expandNodeClass + "\" id=\"" + cfg.targetID + "_AX_bodyNodeIndent_AX_" + r + "_AX_" + CHidx + "_AX_" + itemIndex + "\" style=\"" + CH.align + ":" + (indentWidth - 40) + "px;");
						...
						if (CH.getIconClass) {  // + expandNodeClass + 추가됨
							iconClass = getIconClassValue(CH.getIconClass, item, itemIndex, item[CH.key], CH.key, CH);
							tpo.push("<a class=\"bodyNodeIcon " + iconClass + expandNodeClass + "\" id=\"" + cfg.targetID + "_AX_bodyNodeIcon_AX_" + r + "_AX_" + CHidx + "_AX_" + itemIndex + "\" style=\"" + CH.align + ":" + (indentWidth-20) + "px;\"></a>"); // -20 추가
						}
						if ((hasFixed && !CH.isFixedCell) || !hasFixed || isfix != undefined) {
							/*
							 * 문자열 길이를 통해 최대 너비를 구하는 식을 추가함 - jhko (14.06.18)
							 */
							var valueText = '';
							if (CH.formatter) {
								valueText = getFormatterValue(CH.formatter, item, itemIndex, item[CH.key], CH.key, CH);
								tpo.push(valueText);
							} else {
								valueText = item[CH.key];
								tpo.push(valueText);
							}
							var textLength = indentWidth + (valueText.length * cfg.textLengthConstant);
							if(cfg.maxWidth < textLength){
								cfg.maxWidth =  textLength;
							}
						} else {
							tpo.push("&nbsp;");
						}
	...
	expandToggleList: function (itemIndex, item, r, c) {
		...
		if (item[cfg.reserveKeys.openKey]) {
			...
			for (var r = 0; r < cfg.body.rows.length; r++) {
				...
				axdom("#" + cfg.targetID + "_AX_tr_" + r + "_AX_n_AX_" + itemIndex).find(".bodyNodeIcon").removeClass("expand");  // 추가됨
			}
			...
			axf.each(this.list, function (itemIndex, item) {
				if(!item.AXTreeSplit){
					if (pHash == item[cfg.reserveKeys.parentHashKey].left(pHash.length)) {
						// 중복 추가 안되도록 수정 - jhko (14.06.18)
						if(jQuery.inArray(item[cfg.reserveKeys.parentHashKey], removepHashs) == -1 ){
			...
			for (var r = 0; r < cfg.body.rows.length; r++) {
				...
				axdom("#" + cfg.targetID + "_AX_tr_" + r + "_AX_n_AX_" + itemIndex).find(".bodyNodeIcon").addClass("expand"); // 추가됨
			}
		...
		if (isoncheck) { //체크박스 구현
			...
			if (myTarget) {
				...
				if (clickNodeType == "bodyNodeIndent") {
					...
					//root 선택 안되도록 변경 (대상 선택)
					if ((clickNodeType == "nbody" || clickNodeType == "fbody") && this.list[itemIndex].type != 'root') {
						if (event.shiftKey) {

						}					 
						else if (event.ctrlKey && cfg.multiSelect) {  //multiSelect 지원 & 옵션화
							if (this.selectedRow.length > 0) {
								var body = this.body;
								axf.each(this.selectedRow, function () {
									body.find(".gridBodyTr_" + this).removeClass("selected");
	...
	gridBodyDBLClick: function (event) {
		...
		if (myTarget && this.list[itemIndex].type != 'root') {
			//colHeadTool ready
	...
	contentScrollResize: function (resetLeft) {
		...
		/*
		 *  문자열 길이에 따라 너비 변경
		 *  정확한 길이를 측정하여 반영하면 처리하는데 오래걸려 간단하게 글자수에 10을 곱함
		 */
		/*
		maxWidth = Math.max.apply( null, $('.gridBodyTr:not([style="display: none;"]) .bodyTdText').map( function () {
			var text_width = $(this).text().length*9.5;
			return (text_width + parseInt($(this).css("padding-left"))); //($(this).text().length*20);// +  parseInt($(this).css("padding-left"));
		}).get() );
		*/
		var _maxWidth = this.config.maxWidth;
		if(_maxWidth > document.body.clientWidth-17){
			axdom("#AXPage").css({width:_maxWidth}); 		
			$("colgroup col").css("width", _maxWidth);
		}else{
			_maxWidth = document.body.clientWidth-17;
			axdom("#AXPage").css({width:_maxWidth}); 		
			$("colgroup col").css("width", _maxWidth);
		}
		// 여기까지 추가된 부분
		...
		this.colHead.css({ width: scrollWidth + axdom("#" + cfg.targetID + "_AX_scrollTrackXY").width() });  // colHead width 재정의
		...
		if (scrollWidth > bodyWidth && cfg.xscroll) {
			...
			var scrollTrackXWidth = bodyWidth - axdom("#" + cfg.targetID + "_AX_scrollTrackXY").width();
			axdom("#" + cfg.targetID + "_AX_scrollTrackX").css({ width: scrollTrackXWidth });
			var scrollXHandleWidth = ((bodyWidth - axdom("#" + cfg.targetID + "_AX_scrollTrackXY").outerWidth()) * scrollTrackXWidth) / (scrollWidth + 50);
			...
			if (cfg.height == "auto") {
				...
				// if문 추가됨. ie5에서 height가 0이 되는 현상 발생
