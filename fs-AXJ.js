    ...
    function toDate(separator, defaultDate) {
        if (this.length == 10) {
            ...
            try {
                ...
                var is24 = true;
                if (aTime.right(2) == amTitle || aTime.right(2) == pmTitle ) {
                    is24 = false;
                }
...
var AXCalendar = Class.create(AXJ, {
    ...
    printDayPage: function (date) {
        ...
        var i = 0; while (i < 6) {
            ...
            var k = 0; while (k < 7) {
                ...
                //if (setDate.diff(roopDate, "D") == 0) tdClass.push("setDate");				//이전 클릭 날짜 표시 삭제              
                //if (basicDate.diff(roopDate, "D") == 0) tdClass.push("setDate");				//기존 선택 날짜 표시 (초기값 : today)
                if (nowDate.diff(roopDate, "D") == 0) tdClass.push("setDate");				//오늘 날짜 표시 (초기값 : today)
    ...
    setTimeDisplay: function (objID, objSeq, objType){
    ...
    printMonthPage: function (date) {
        ...
        var i = 0; while (i < 4) {
            ...
            var k = 0; while (k < 3) {
                ...
                po.push("<td class=\"bodyCol_" + k + " bodyRow_" + i + " " + tdClass.join(" ") + "\"><a " + cfg.href + " class=\"calendarMonth\" id=\"" + cfg.targetID + "_AX_" + m + "_AX_month\" title=\"\">" + printmonth + monthTitle +"</a></td>");
                k++;
                m++;
            }
            po.push("</tr>");
    ...
    printYearPage: function (year) {
        ...
        var i = 0; while (i < 4) {
            ...
            var k = 0; while (k < 3) {
                ...
                po.push("<td class=\"bodyCol_" + k + " bodyRow_" + i + " " + tdClass.join(" ") + "\"><a " + cfg.href + " class=\"calendarMonth\" id=\"" + cfg.targetID + "_AX_" + m + "_AX_year\" title=\"\">" + m + yearTitle + "</a></td>");
                k++;
                m++;
            }
            po.push("</tr>");
    ...
    printTimePage: function (displayTime) {
        ...
        if (displayTime) {											/* edit   AMPM to 오전오후*/
            var now = displayTime.split(":");            
            //var hh = now[0].setDigit(2);
            var myDate = now[0].substring(0,10);
            var hh = now[0].substring(11,13).setDigit(2);
            var mm = now[1].left(2).setDigit(2);
            //var apm = now[1].right(2);
            if (hh == "00" && mm == "00") {										/* edit e - 00:00 일경우 24:00 금지*/
                hh = "00";
               // apm = amTitle;
            }
            //if (apm == "00") apm = amTitle;
        } else {
            var now = new Date();
            var hh = now.getHours();
            var mm = now.getMinutes();
            //var apm = amTitle;
            if (hh == 0 && mm == 0) {												/* edit e - 00:00 일경우 24:00 금지*/
                hh = 0;
            }
           /* if (hh > 12) {
                apm = pmTitle;
        ...
        po.push("<div class='hourTitle'>" + hourTitle + "</div>");
        po.push("<div class='hourSlider'><input type='text' value='" + hh + "' id='" + cfg.targetID + "_AX_hour' class='AXInput' /></div>");
        po.push("<div class='minuteTitle'>"+ minuteTitle + "</div>");
        po.push("<div class='minuteSlider'><input type='text' value='" + mm + "' id='" + cfg.targetID + "_AX_minute' class='AXInput' /></div>");
       po.push("<div id='" + cfg.targetID + "_AX_timeDisplay' class='timeDisplay'>"+ myDate +" " + hh + ":" + mm + "</div>");
        																																									/* temp : add input box*/
     //   po.push("<div class='timeDisplay'> <input type=\"text\" name=\"NumberValue\" value='" + hh + ":" +  mm + " " + apm +  "' id='" + cfg.targetID + "_AX_timeInput' class='AXInputW60' /></div>");

     // po.push("<input type='text' class='timeDisplay' name = \"NumberValue\" value='" + hh + ":" +  mm + " " + apm +  "' id='" + cfg.targetID + "_AX_timeInput'  />");		/* edit  - timeDisplay 입력가능하도록 -> .text로 넣을수 없고 .val로 값 넣어야*/
		...
		axdom("#inputBasic_AX_" + cfg.targetID + "_AX_hour_AX_SliderHandleTitle").unbindInput();
		axdom("#inputBasic_AX_" + cfg.targetID + "_AX_minute_AX_SliderHandleTitle").unbindInput();

        axdom("#" + cfg.targetID + "_AX_hour").bindSlider({										
            min: 0, max: 23, onChange: function (objID, objVal) {							/* add  - 0~23시로 변경 */
        ...
        /*axdom("#" + cfg.targetID + "_AX_AMPM").bindSwitch({
            off: amTitle, on: pmTitle, onChange: function (objID, objVal) {
            	if( this.value == pmTitle){
            		axdom("#" + cfg.targetID + "_AX_hour_AX_AnchorSliderMaxTitle").text("11");
            	}else{
            		axdom("#" + cfg.targetID + "_AX_hour_AX_AnchorSliderMaxTitle").text("12");
            	}	             
                timePageChange(objID, objVal);
            }
        });*/
		...
		axdom("#inputBasic_AX_" + cfg.targetID + "_AX_hour_AX_SliderHandleTitle").bindNumber({								// add  - 시간입력에 따른 실시간 설정값 및 화면 변경
			 min: 0, max: 23, onChange: function (objID, objVal) {							

				objID = cfg.targetID + "_AX_hour";

				if (objVal == undefined) {
					objVal = axdom("#inputBasic_AX_" + cfg.targetID + "_AX_hour_AX_SliderHandleTitle").val();
				}

				//var apm = axdom("#" + cfg.targetID + "_AX_AMPM").val();

				var maxhour = 23;
				//if(apm==pmTitle) maxhour=11;

				if (objVal.number() < 0) objVal = 0;
				else if (objVal.number() > maxhour) objVal = maxhour;
				var valueWidth = maxhour - 0;
				var pixelWidth = 83;
				var pixelLeft = ((objVal - 0) * pixelWidth) / valueWidth;

				axdom("#inputBasic_AX_" + cfg.targetID + "_AX_hour_AX_SliderHandle").css({ left: pixelLeft - (24 / 2) });
				axdom("#inputBasic_AX_" + cfg.targetID + "_AX_hour_AX_SliderInside").css({ width: pixelLeft });
				axdom("#inputBasic_AX_" + cfg.targetID + "_AX_hour_AX_SliderHandleTitle").css({ left: pixelLeft - (30 / 2) });				
				axdom("#inputBasic_AX_" + cfg.targetID + "_AX_hour_AX_SliderHandleTitle").val(objVal);
				axdom("#inputBasic_AX_" + cfg.targetID + "_AX_hour").val(objVal);
				axdom("#" + cfg.targetID + "_AX_hour").val(objVal);
		...
		axdom("#inputBasic_AX_" + cfg.targetID + "_AX_minute_AX_SliderHandleTitle").bindNumber({								// add  - 분 입력에 따른 실시간 설정값 및 화면 변경
			 min: 0, max: 59, onChange: function (objID, objVal) {							

				objID = cfg.targetID + "_AX_minute";

				if (objVal == undefined) {
					objVal = axdom("#inputBasic_AX_" + cfg.targetID + "_AX_minute_AX_SliderHandleTitle").val();
				}
				...
				if (objVal.number() < 0) objVal = 0;
				else if (objVal.number() > maxminute) objVal = maxminute;
				var valueWidth = maxminute - 0;
				var pixelWidth = 83;
				var pixelLeft = ((objVal - 0) * pixelWidth) / valueWidth;



				axdom("#inputBasic_AX_" + cfg.targetID + "_AX_minute_AX_SliderHandle").css({ left: pixelLeft - (24 / 2) });
				axdom("#inputBasic_AX_" + cfg.targetID + "_AX_minute_AX_SliderInside").css({ width: pixelLeft });
				axdom("#inputBasic_AX_" + cfg.targetID + "_AX_minute_AX_SliderHandleTitle").css({ left: pixelLeft - (30 / 2) });				
				axdom("#inputBasic_AX_" + cfg.targetID + "_AX_minute_AX_SliderHandleTitle").val(objVal);
				axdom("#inputBasic_AX_" + cfg.targetID + "_AX_minute").val(objVal);
				axdom("#" + cfg.targetID + "_AX_minute").val(objVal);
    ...
    timePageChange: function (objID, objVal) {
        ...
        var cfg = this.config;
        var ccfg = axdom("#"+ this.config.targetID);
 
       /* if(axdom("#" + cfg.targetID + "_AX_AMPM").val() == pmTitle){ 	
        ...
        var myDate = axdom("#" + cfg.targetID + "_AX_box").find(".timeDisplay").text().substring(0,10);        
        
        axdom("#" + cfg.targetID + "_AX_box").find(".timeDisplay").text(myDate + " " + mytime);
...
var AXContextMenuClass = Class.create(AXJ, {
    initialize: function (AXJ_super) {
        ...
        this.config.responseiveMobile = 0; /* 모바일 반응 너비 */
    },
    init: function () {
...
var AXMobileModal = Class.create(AXJ, {
    ...
    setSizeModal: function (configs, onLoad) {
        ...
        var cssStylesStart = {
            ...
            top: (axf.clientHeight() - (modalHeight*0.8)) / 2 + axf.scrollTop(),
