...
var AXInputConverter = Class.create(AXJ, {
	version: "AXInputConverter v1.45",
	...
	appendAnchor: function (objID, objSeq, bindType) {
		...
		t = t + marginH;
		/*t = t;*/
		w = iobj.outerWidth()  + 23;			/* edit   */
		h = iobj.outerHeight();
		
		var outerH = obj.bindTarget.outerHeight(); 	/* Edit   */ 
		
		//System.out.pringln("outerH="+outerH + ", marginH="+marginH);
		//alert("outerH="+outerH + ", marginH="+marginH);
	...
	alignAnchor: function (objID, objSeq) {
		...
		//var marginH = iobj.css("margin-top").number();  /* edit   */
		
		var w = obj.bindTarget.outerWidth() + 23;
		//var h = obj.bindTarget.outerHeight() - 3;		// 멀티브라우저 지원시 -3 삭제 요망
		...
		if (obj.bindType == "placeHolder") {
			...
			var handleH = h + 20;
			axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_dateHandle").css({ height: handleH });
			
		} else if (obj.bindType == "twinDateTime") {
			
				//var handleW = w - 100;
				var handleH = h + 20;
				//axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_dateHandle").css({ width : handleW, height: handleH });
				axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_dateHandle").css({height: handleH });
		}
	},
	...
	/* number ~~~~~~~~~~~~~~~ */
	bindNumber: function (objID, objSeq) {													/*edit   */
		var cfg = this.config;
		var obj = this.objects[objSeq];
...
//	},
	...
	bindNumberCheck: function (objID, objSeq) {
		...
		if (maxval != undefined && maxval != null) {
			if ((nval) > maxval) {
				//axdom("#" + objID).val("");																/*edit   - 빈칸만들기 - 임계값으로 만들기*/
				axdom("#" + objID).val(maxval);
				try {
					//this.msgAlert("설정된 최대값을 넘어서는 입력입니다.");					/* edit   - msgAlert 삭제 */
				} catch (e) { }
			} else {
				if (minval != undefined && minval != null) {
					if ((nval) < minval) {
						//axdom("#" + objID).val("");													/*edit   - 빈칸만들기 - 임계값으로 만들기*/
						axdom("#" + objID).val(minval);
						try {
							//this.msgAlert("설정된 최소값보다 작은 입력입니다.");			/* edit   - msgAlert 삭제 */
						} catch (e) { }
					} else {
	...
	bindSlider: function (objID, objSeq) {
		...
		//alert("objVal.number().money() + obj.config.unit = " +objVal.number().money() + obj.config.unit);
		
		var tmpHour = objVal.number().money() + obj.config.unit;		
		po.push("		<div id=\"" + cfg.targetID + "_AX_" + objID + "_AX_SliderInside\" class=\"AXanchorSliderBarInside\">");
		po.push("<input type='text'  name='NumberValue' id=\"" + cfg.targetID + "_AX_" + objID + "_AX_SliderHandleTitle\" class=\"AXanchorSliderHandleTitle\" data-axbind=\"number\" value=" + tmpHour +" / ></div>");
		//po.push("<input type='text' class='timeDisplay' name = \"NumberValue\" value='" + hh + ":" +  mm + " " + apm +  "' id='" + cfg.targetID + "_AX_timeInput'  />");
		
		//alert("#" + cfg.targetID + "_AX_" + objID + "_AX_SliderHandleTitle");
//		
//		<input type="number" name="numberValue" id="AXInputNumber" />
//		<script>
//			axdom("#AXInputNumber").bindNumber({min:1, max:12} );
//		</script>		
		
		
		po.push("		<a " + obj.config.href + " id=\"" + cfg.targetID + "_AX_" + objID + "_AX_SliderHandle\" class=\"AXanchorSliderHandle\">handle</a>");
		po.push("	</div>");
		//po.push("	<div id=\"" + cfg.targetID + "_AX_" + objID + "_AX_AnchorSliderMaxTitle\" class=\"AXanchorSliderMaxTitle\">" + obj.config.max.number().money() + obj.config.unit + "</div>");
		po.push("	<div id=\"" + objID + "_AX_AnchorSliderMaxTitle\" class=\"AXanchorSliderMaxTitle\">" + obj.config.max.number().money() + obj.config.unit + "</div>"); 	/*edit   */
		po.push("</div>");
	...
	bindSliderMouseMove: function (objID, objSeq, event) {
		...
		axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_SliderHandleTitle").css({ left: stX });							/* edit   // 슬라이더 위치에 따라 위치 따라움직이기*/
		
		//axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_SliderHandleTitle").text(objVal.number().money() + obj.config.unit);
		axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_SliderHandleTitle").val(objVal.number().money() + obj.config.unit); 	/* edit   // 슬라이더 이동시 표시시간 값 변경 (input용)*/
	...
	bindOnlySliderSetValue: function (objID, objSeq, value) {										
	...
	bindSliderSetValue: function (objID, objSeq, value) {
		...
		axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_SliderHandle").css({ left: pixelLeft - (obj.config._handleWidth / 2) });
		axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_SliderInside").css({ width: pixelLeft });
		axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_SliderHandleTitle").css({ left: pixelLeft - (obj.config._maxTitleWidth / 2) });				/*edit   //슬라이더 수치변경값 초기위치 설정*/
		axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_SliderHandleTitle").val(objVal.number().money() + obj.config.unit);
	...
	bindDate: function (objID, objSeq) {
		...
		po.push("<a " + obj.config.href + " id=\"" + cfg.targetID + "_AX_" + objID + "_AX_dateHandle\" class=\"" + cfg.anchorDateHandleClassName + "\" style=\"right:0px;top:0px;\">handle</a>");
		axdom("#" + cfg.targetID + "_AX_" + objID).append(po.join(''));
		axdom("#" + cfg.targetID + "_AX_" + objID).show();
	...
	bindDateExpand: function (objID, objSeq, isToggle, event) {
		...
		po.push("	<div style=\"padding-top:5px;\" align=\"center\">");																															/*edit   - single date - add OK button*/
		...
		if (obj.config.expandTime) { //시간 선택 기능 확장시
			obj.nDate = myDate;
																									...
																									//Edit   - Single달력에서 시간변화에따른 실시간 적용 삭제
			...
			//var apm = amTitle;																/* Edit   AMPM to 오전오후*/
			...
			obj.mycalendartime.printTimePage(printDate + " " + myHH.setDigit(2) + ":" + myMI.setDigit(2));
		}
...
/*			if (myHH > 12) {
	...
	bindDateMobileModalBodyClick: function (objID, objSeq, event) {
		...
		if (myTarget) {
			...
			if (act == "date") {
				...
				axdom("#" + objID).val(printDate);
				//obj.modal.close();
				//this.bindDateExpandClose(objID, objSeq, event);								/* 날짜를 눌렀을때 바로 닫히는 기능 제거  OK눌렀을때 적용되도록 */
			} else if (act == "month") {
	...
	bindDateExpandOnlyClose: function (objID, objSeq, event) {  /*edit   for single date*/
	...
	bindDateExpandBoxClick: function (objID, objSeq, event) {
		...
		var myTarget = this.getEventTarget({
			...
			find: function (evt, evtIDs) {
				if (!evt.id) {return false;}																	// add   - event id 확인용
		...
		if (!isDateClick) {
			//this.bindDateExpandClose(objID, objSeq, event);	/* Edit   - single의  달력 바깥 클릭시 아무동작 안하게 하기위해 */
			this.bindDateExpandOnlyClose(objID, objSeq, event);
		} else {
			...
			if (ename == "expandPrev") {
				...
				//axdom("#" + objID).val(printDate);									/* Edit   - sigle 에서 달력일자 클릭시 실시간 적용되지 않고 OK적용되도록 하기위해 삭제*/
				//this.bindDateExpandClose(objID, objSeq, event);					/* Edit   - single 에서의 달력 일자 클릭시 expand obj 종료되지 않도록 하기위해 삭제 */
			
				obj.mycalendar.dayPageSetDay(obj.nDate);     /* Edit  e - siingle에서 달력일자 클릭시 마킹하기 위해 추가*/
			...
			} else if(ename == "closeButton"){						/* Edit   - OK 버튼 추가 후 적용/종료 이벤트 등록*/
	...
	bindDateChangePage: function (objID, objSeq, setDate, pageType) {
		...
		if (obj.config.selectType == "y") {
			...
			//axdom("#" + objID).val(printDate);																			/* Edit   - OK클릭시 적용하기 위해 실시간 적용 삭제*/
		}
	},
	/* twinDate ~~~~~~~~~~~~~~~~~ */
	...
	bindTwinDateExpand: function (objID, objSeq, isToggle, event) {
		...
		po.push("	<div>");
		po.push("		<table cellpadding=\"0\" cellspacing=\"0\" style=\"margin-bottom:0px;\">");
		po.push("			<tbody>");
		po.push("				<tr>");
		...
		if (obj.config.expandTime) { //시간 선택 기능 확장시
		po.push("					<div class=\"timeDisplayBox\" id=\"" + cfg.targetID + "_AX_" + objID + "_AX_displayTimeBox2\"></div>");		/* temp*/
//			<input type="time" name="usr_time">
			//po.push("					<input type=\"time\"  id=\"" + cfg.targetID + "_AX_" + objID + "_AX_displayTimeBox2\"></div>");
		}
		po.push("					</td>");
		po.push("				</tr>");
		po.push("			</tbody>");
		...
		if (obj.config.expandTime) { //시간 선택 기능 확장시
			...
			/*if (myHH > 12) {
				apm = pmTitle;
				axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_displayTimeBox1_AX_hour_AX_AnchorSliderMaxTitle").text(11);
				myHH -= 12;
			}*/
			//obj.mycalendartime1.printTimePage(myHH.setDigit(2) + ":" + myMI.setDigit(2) + " " + apm);
			
			var printDate1 = obj.nDate1.print("yyyy" + separator + "mm" + separator + "dd");			
			obj.mycalendartime1.printTimePage(printDate1 + " " + myHH.setDigit(2) + ":" + myMI.setDigit(2));
			...
			obj.nDate2 = myDate2;	
																									// Edit   - 시간변경 실시간 반영 삭제 (기간달력 중 종료 시간)
			var mycalendartimeChange2 = this.bindTwinDateTimeChange.bind(this);
			obj.mycalendartimeChange2 = function (myTime) {														/* 시간변경시 사용할 수 있는 함수*/
			...
			var printDate2 = obj.nDate2.print("yyyy" + separator + "mm" + separator + "dd");			
			obj.mycalendartime2.printTimePage(printDate2 + " " + myHH.setDigit(2) + ":" + myMI.setDigit(2));
			//obj.mycalendartime2.printTimePage(myHH.setDigit(2) + ":" + myMI.setDigit(2) + " " + apm);
		}
		
		/*if(apm == pmTitle){
			axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_displayTimeBox2_AX_hour_AX_AnchorSliderMaxTitle").text(11);
		}*/
		...
		if(obj.nDate1.diff(obj.nDate2) < 0){														/*add    처음시작시 강제입력된 내용 확인 후 시작-끝 순서 동기*/	
			obj.nDate2 = obj.nDate1;																// 순서가 맞지 않을 시 무조건 시작일에 종료일을 맞춤
			//alert(obj.nDate1 +"    "+ obj.nDate2);
			//alert("시작-종료일시를 확인해 주세요");
			this.bindTwinDateChangePage(objID, objSeq, 2, obj.nDate1, "d");
		}	
	...
	bindTwinDateTimeChange: function (objID, objSeq, myTime, seq) {					////////////////
		...
		if (obj.config.expandTime) {
			if( printDate1 == printDate2 ){																			/* add   - 같은 날일경우 시작 / 종료 시간 확인*/
				var timeTmp1 = obj.mycalendartime1.getTime();				
				var timeTmp2 = obj.mycalendartime2.getTime();
				var i = 0;				
				for(i=0; i<timeTmp1.length; i++){
					
					    if((timeTmp1.substr(i,1) - timeTmp2.substr(i,1)) < 0){
							break;
						}
						else if( (timeTmp1.substr(i,1) - timeTmp2.substr(i,1) ) > 0 ){
							isinvalidtime = true;	
							break;
						}
				}
		...
		if(isinvalidtime){					
			if (seq == 1) { // 시작시간이 이 바뀌었다면,				
				//var apm = axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_displayTimeBox1_AX_AMPM").val();				
				
				var hh = axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_displayTimeBox1_AX_hour").val().number();
           		var mi = axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_displayTimeBox1_AX_minute").val().number();
           		obj.mycalendartime2.printTimePage( printDate1 + " " + hh.setDigit(2) + ":" + mi.setDigit(2));
            	//obj.mycalendartime2.printTimePage(hh.setDigit(2) + ":" + mi.setDigit(2) + " " + apm);
            	
            	/*if(apm==pmTitle){           		
            		axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_displayTimeBox2_AX_hour_AX_AnchorSliderMaxTitle").text("11");
            	}else{
            		axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_displayTimeBox2_AX_hour_AX_AnchorSliderMaxTitle").text("12");
            	}*/
            	
			}else{	//종료시간이 바뀌었다면
				//var apm = axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_displayTimeBox2_AX_AMPM").val();
				var hh = axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_displayTimeBox2_AX_hour").val().number();
				var mi = axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_displayTimeBox2_AX_minute").val().number();
				obj.mycalendartime1.printTimePage(printDate2 + " " + hh.setDigit(2) + ":" + mi.setDigit(2));				
				
				//obj.mycalendartime1.printTimePage(hh.setDigit(2) + ":" + mi.setDigit(2) + " " + apm);
				
				/*if(apm==pmTitle){
					axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_displayTimeBox1_AX_hour_AX_AnchorSliderMaxTitle").text("11");
            	}else{
            		axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_displayTimeBox1_AX_hour_AX_AnchorSliderMaxTitle").text("12");
            	}*/
			}
		}
	...
	bindTwinDateExpandOnlyClose: function (objID, objSeq, event) {
		var obj = this.objects[objSeq];
		var cfg = this.config;
	...
	bindTwinDateExpandClose: function (objID, objSeq, event) {
		...
		if (AXgetId(cfg.targetID + "_AX_" + objID + "_AX_expandBox")) {
			...
			if (obj.config.selectType == "y") {
				...
				var dateTmp1 = printDate1;																			/* add   - 같은 날짜인지 확인을 위한 변수*/
				var dateTmp2 = printDate2;
				...
				if (obj.config.expandTime) {					
					...
					if( dateTmp1 == dateTmp2 ){																			/* add   - 같은 날일경우 시작 / 종료 시간 확인*/
						var timeTmp1 = obj.mycalendartime1.getTime();
						var timeTmp2 = obj.mycalendartime2.getTime();
						
						for(var i=0; i<timeTmp1.length; i++){
												
								if((timeTmp1[i] - timeTmp2[i]) < 0){
									break;
								}
								else if( (timeTmp1[i] - timeTmp2[i]) > 0 ){
	...
	bindTwinDateExpandBoxClick: function (objID, objSeq, event) {
		...
		isDateClick = (myTarget) ? true : false;
		if (!isDateClick) {
			//this.bindTwinDateExpandClose(objID, objSeq, event);	/* Edit   - Twin의 달력 바깥 클릭시 아무동작도 하지 않도록 삭제 */
			this.bindTwinDateExpandOnlyClose(objID, objSeq, event);
		} else {
			var ids = myTarget.id.split(/_AX_/g);
			var ename = ids.last();
			...
			if (ename == "expandPrev1") {
				if (obj.mycalendarPageType == "d") {
					...
					obj.mycalendar1.dayPageSetDay(obj.nDate1);						// add   - 월이동시 날짜 선택됨 추가
					
					var printDate = obj.nDate1.print("yyyy" + separator + "mm" + separator + "dd");				
					var printTime = obj.mycalendartime1.getTime();
					obj.mycalendartime1.printTimePage(printDate + " " + printTime);
				...
				if (obj.mycalendarPageType == "d") {
					...
					obj.mycalendar2.dayPageSetDay(obj.nDate2);						// add   - 월이동시 날짜 선택됨 추가	
					if (obj.config.expandTime) {
						this.bindTwinDateTimeChange(objID, objSeq, nDate2, 2);			// add   - 월이동시 시간 전후관계 확인 / 동기화 추가
					}
				} else if (obj.mycalendarPageType == "m") {
				...
				if (obj.mycalendarPageType == "d") {
					this.bindTwinDateChangePage(objID, objSeq, 1, nDate1.add(1, "m"), "d");
					obj.mycalendar1.dayPageSetDay(obj.nDate1);						// add   - 월이동시 날짜 선택됨 추가
					//obj.bindTwinDateTimeChange(objID, objSeq, nDate1, 1);
					if (obj.config.expandTime) {
						this.bindTwinDateTimeChange(objID, objSeq, nDate1, 1);			// add   - 월이동시 시간 전후관계 확인 / 동기화 추가
											}
				} else if (obj.mycalendarPageType == "m") {					
					this.bindTwinDateChangePage(objID, objSeq, 1, nDate1.add(1, "y"), "m");
				...
				if (boxType == "displayBox1") {
					...
					//axdom("#" + obj.config.startTargetID).val(printDate);											/* edit   - 페이지에 twin 시작일 실시간 반영 삭제 */
					this.bindTwinDateChangePage(objID, objSeq, 1, obj.nDate1, "d");						/* Add 다른의 날짜 선택시 월 페이지 자동이동 */	
					...
					if (obj.config.expandTime) {
						...
						this.bindTwinDateTimeChange(objID, objSeq, nDate1, 1);			// add   -  일 선택시 시간 전후관계 확인 / 동기화 추가
					}
					...
					//axdom("#" + objID).val(printDate);																/* edit   - 페이지에 twin 종료일 실시간 반영 삭제  */
					this.bindTwinDateChangePage(objID, objSeq, 2, obj.nDate2, "d");						/* Add 다른의 날짜 선택시 월 페이지 자동이동 */
					...
					if (obj.config.expandTime) {
						...
						var printDate = obj.nDate2.print("yyyy" + separator + "mm" + separator + "dd");				
						var printTime = obj.mycalendartime2.getTime();
						obj.mycalendartime2.printTimePage(printDate + " " + printTime);
						
						this.bindTwinDateTimeChange(objID, objSeq, nDate2, 2);			// add   -  일 선택시 시간 전후관계 확인 / 동기화 추가
					}
				}

				if (obj.nDate1.diff(obj.nDate2) < 0) {																							/*  초기 두 날짜 비교 부분 later than */
					if (boxType == "displayBox1") {
						...
						//this.bindTwinDateChangePage(objID, objSeq, 1, "m", "d");
				...
				if (boxType == "displayBox1") {
					if (obj.config.selectType == "m") {						
						...
						//axdom("#" + obj.config.startTargetID).val(printDate);					/* edit   - 페이지에 twin 시작월 실시간 반영 삭제 */
						//this.bindTwinDateExpandClose(objID, objSeq, event);
						obj.mycalendar1.monthPageSetMonth(obj.nDate1);
						obj.mycalendar1.dayPageSetDay(obj.nDate1);						// add   - 월 선택시 날짜 선택됨 추가	
						if (obj.config.expandTime) {
							this.bindTwinDateTimeChange(objID, objSeq, nDate1, 1);			// add   -  월 선택시 시간 전후관계 확인 / 동기화 추가
						}
					} else {
						...
						obj.mycalendar1.dayPageSetDay(obj.nDate1);						// add   - 월 이동시 날짜 선택됨 추가
						if (obj.config.expandTime) {
							this.bindTwinDateTimeChange(objID, objSeq, nDate1, 1);			// add   -  월 선택시 시간 전후관계 확인 / 동기화 추가
						}
					}
					
					if(obj.nDate1.diff(obj.nDate2) < 0){														/*add   달력1에서 월 이동시 시작-끝 순서 동기*/						
						nDate2 = nDate1;
						this.bindTwinDateChangePage(objID, objSeq, 2, obj.nDate1, "d");
						obj.mycalendar2.dayPageSetDay(obj.nDate2);	
						if (obj.config.expandTime) {
							this.bindTwinDateTimeChange(objID, objSeq, nDate1, 1);			// add   -  월 선택 시 시간 전후관계 확인 / 동기화 추가
						}
					}
					...
					if (obj.config.selectType == "m") {
						...
						//axdom("#" + objID).val(printDate);										/* edit   - 페이지에 twin 종료월 실시간 반영 삭제 */
						obj.mycalendar2.monthPageSetMonth(obj.nDate2);
						obj.mycalendar2.dayPageSetDay(obj.nDate2);						// add   - 월이동시 날짜 선택됨 추가	
						if (obj.config.expandTime) {
							this.bindTwinDateTimeChange(objID, objSeq, nDate2, 2);			// add   -  월 선택시 시간 전후관계 확인 / 동기화 추가
						}
					} else {
						var yy = nDate2.getFullYear();
						var dd = nDate2.getDate();
					...
					if(obj.nDate1.diff(obj.nDate2) < 0){														/*add   달력2에서 월 이동시 시작-끝 순서 동기*/
						nDate1 = nDate2;
						this.bindTwinDateChangePage(objID, objSeq, 1, obj.nDate2, "d");
						obj.mycalendar1.dayPageSetDay(obj.nDate1);						// add   - 월이동시 날짜 선택됨 추가	
						if (obj.config.expandTime) {
							this.bindTwinDateTimeChange(objID, objSeq, nDate2, 2);			// add   -  월 선택시 시간 전후관계 확인 / 동기화 추가
						}
				...
				if (boxType == "displayBox1") {
					if (obj.config.selectType == "y") {
						...
						//axdom("#" + obj.config.startTargetID).val(printDate);				/* edit   - 페이지에 twin 시작년 실시간 반영 삭제 */
						//this.bindTwinDateExpandClose(objID, objSeq, event);
					...
					if (obj.config.selectType == "y") {
						...
						//axdom("#" + objID).val(printDate);									/* edit   - 페이지에 twin 종료년 실시간 반영 삭제 */
						//this.bindTwinDateExpandClose(objID, objSeq, event);
			...
			if (obj.nDate1.diff(obj.nDate2) < 0) {															/*Edit  - 동기화*/
				if (  (ename == "expandNext1") || (ename == "controlMonth1") || (ename == "controlYear1") || (ename == "year")  ) {
						...
						var apm = amTitle;
						var myTimes = nDate1.print("hh:mi").split(":");
						var myHH = myTimes[0].number();
						var myMI = myTimes[1];
						if (myHH > 12) {
							apm = pmTitle;
						...
						if (obj.config.expandTime) {
							this.bindTwinDateTimeChange(objID, objSeq, nDate1, 1);	//시간 전후관계 확인 / 동기화 추가
						}
						
				    }else if (  (ename == "expandPrev2") || (ename == "controlMonth2") || (ename == "controlYear2")   ) {
						nDate2 = nDate1;
						this.bindTwinDateChangePage(objID, objSeq, 1, obj.nDate2, "d");
						obj.mycalendar1.dayPageSetDay(obj.nDate1);
						if (obj.config.expandTime) {
							this.bindTwinDateTimeChange(objID, objSeq, nDate2, 2);		//시간 전후관계 확인 / 동기화 추가
						}
				}																														/*년, 월 직접클릭 변경은 그쪽에 삽입*/
			}	
			...
			if (clearDate) {
				...
				if (obj.config.selectType == "y") {
					...
					if (obj.config.expandTime) {
						...
						var hh = va.substr(8, 2).number();
						var mm = va.substr(10, 2).number();
						printDate += " " + hh.setDigit(2) + ":" + mm.setDigit(2);												
					}
					...
					if (obj.nDate2 == undefined) {
						...
						if (obj.config.expandTime) {
							...
							var hh = va.substr(8, 2).number();
							var mm = va.substr(10, 2).number();
							printDate += " " + hh.setDigit(2) + ":" + mm.setDigit(2);																										// readonly-직접입력 금지로, 시간 선후관계 체크 불필요
						}
						axdom("#" + objID).val(printDate);
					}
